...
export async function readFinderFeed({key,offset=0,fetcher=fetch}){
 ...
 try{
  const response=await fetcher(`${apiOrigin}/api/integrations/railway/listings?limit=24&offset=${offset}`,{headers:{'X-API-Key':key,Accept:'application/json'},cache:'no-store',redirect:'error',signal:AbortSignal.timeout(20000)});
  ...
  if(!Array.isArray(page.items)||page.items.length>24||page.scope!=='unknown-without-other-sellers'||!Number.isSafeInteger(page.total)||page.total<0||page.offset!==offset||page.limit!==24)throw Error('Invalid Finder response');
  ...
  if(page.items.some(item=>!item||item.source!=='facebook'||item.reviewCategory!=='unknown'||item.otherSellers||item.otherSellersManual||item.otherSellersAutomatic))throw Error('Unexpected seller category');
