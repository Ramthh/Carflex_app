...
// Concurrent viewers of the same page share one short-lived upstream read.
...
export function createFinderFeedReader({read=readFinderFeed,now=Date.now,ttlMs=750,maxEntries=24}={}){
 ...
 return async options=>{
  const id=JSON.stringify([options.key,options.offset??0]),existing=entries.get(id);
  if(existing&&(existing.pending||now()-existing.loadedAt<ttlMs))return existing.promise;
  const entry={pending:true,loadedAt:0,promise:null};
  ...
  entry.promise=Promise.resolve().then(()=>read(options)).then(result=>{
   entry.pending=false;entry.loadedAt=now();
