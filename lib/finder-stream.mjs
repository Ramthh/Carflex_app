...
/** @param {{request:Request,key?:string,cursor:number,filterId?:string|null,filterRevision?:string|null,q?:string,authorize:()=>Promise<boolean>,fetcher?:typeof fetch,authorizeMs?:number,lifetimeMs?:number}} options */
export async function proxyFinderStream({request,key,cursor,filterId=null,filterRevision=null,q='',authorize,fetcher=fetch,authorizeMs=5000,lifetimeMs=45000}){
 ...
 try{q=finderSearchQuery(q);}catch(error){return Response.json({error:error.message},{status:400});}
