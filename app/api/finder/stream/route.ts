...
export async function GET(request:Request){
 try{
  ...
  if([...params.keys()].some(name=>!['cursor','filterId','filterRevision'].includes(name))||params.getAll('cursor').length!==1||params.getAll('filterId').length>1||params.getAll('filterRevision').length>1)return Response.json({error:'Invalid Finder stream.'},{status:400});
  ...
  return await proxyFinderStream({request,key:process.env.CARFLEX_FINDER_API_KEY,cursor,filterId,filterRevision,authorize});
