...
/** @param {{offset?:number,limit?:number,filterId?:string|null,q?:string,onChange?:(page:any)=>void,onReconcile?:()=>void,maxEvents?:number}} options */
export function createFinderLiveState({offset=0,limit=24,filterId=null,q='',onChange,onReconcile,maxEvents=1000}={}){
 ...
 function apply(base,events){
  ...
  for(const event of events){
   ...
   if(!item){
    ...
    else if(event.kind==='remove'&&!filtered)total=Math.max(0,total-1);
    else if(filtered&&event.kind!=='discovery')countPending=true;
    continue;
   }
 ...
 return {
  getCursor:()=>cursor,
  snapshot(next){
   if(closed||(next.selectedFilter?.id||null)!==filterId||(next.searchQuery??'')!==q)return;const at=cursorOf(next.changeCursor);
