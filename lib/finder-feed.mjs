...
export function createFinderFeedReader({read=readFinderFeed,now=Date.now,ttlMs=750,maxEntries=24}={}){
 ...
 return async options=>{
  let q;try{q=finderSearchQuery(options.q);}catch(error){return {status:400,body:{error:error.message}};}
  const id=JSON.stringify([options.key,options.offset??0,options.filterId??null,q]),existing=entries.get(id);
  ...
  entry.promise=Promise.resolve().then(()=>read({...options,q})).then(result=>{
   entry.pending=false;
...
export async function readFinderFeed({key,offset=0,filterId=null,q='',fetcher=fetch}){
 ...
 try{q=finderSearchQuery(q);}catch(error){return {status:400,body:{error:error.message}};}
 try{
  ...
  if((page.searchQuery??'')!==q)throw Error('Wrong car search');
