...
import {finderSearchQuery} from './finder-search.mjs';
...
/** @param {{filterId?:string|null,offset?:number,q?:string,onPage:(page:any)=>void,onLoading:(loading:boolean)=>void,onError:(error:string)=>void,onReset:()=>void,fetcher?:typeof fetch,EventSourceClass?:any,setTimer?:any,clearTimer?:any,now?:()=>number,isVisible?:()=>boolean}} options */
export function createFinderController({filterId=null,offset=0,q='',onPage,onLoading,onError,onReset,fetcher=fetch,EventSourceClass=globalThis.EventSource,setTimer=setTimeout,clearTimer=clearTimeout,now=Date.now,isVisible=()=>true}){
 q=finderSearchQuery(q);
 ...
 const makeState=()=>createFinderLiveState({filterId,offset,q,onChange:page=>{if(active())onPage(page);},onReconcile:reconcile});
 let state=makeState();
 ...
 async function refresh(){
  ...
  try{
   ...
   if((page.searchQuery??'')!==q)throw Error('Finder returned a different car search. Please try again.');
